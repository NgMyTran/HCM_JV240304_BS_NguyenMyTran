...
public class BookManagement {
    ...
    private static void addCatalog(Scanner scanner) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            String catalogName = CatalogService.validateCatalogName(scanner);
            System.out.print("Nhập mô tả danh mục ");
            String descriptions = CatalogService.validateCatalogDescription(scanner);
    ...
    private static void addProduct(Scanner scanner) {
        ...
        int count = Integer.parseInt(scanner.nextLine());
        for (int i = 0; i < count; i++) {
            String productId = ProductService.validateProductId(scanner);

            System.out.print("Nhập tên sản phẩm thứ " + (i + 1) + ":");
            String productName = ProductService.validateProductName(scanner);

            System.out.print("Nhập giá sản phẩm: ");
            double productPrice = ProductService.validateProductPrice(scanner);

            System.out.print("Nhập mô tả sản phẩm: ");
            String description = scanner.nextLine();

            System.out.print("Nhập số lượng sản phẩm ");
            int stock = ProductService.validateProductStock(scanner);

            System.out.println("Chọn danh mục cho sản phẩm:");
            displayAllCatalogs();
            int catalogId = ProductService.validateCatalogId(scanner);
            Catalog catalog = catalogService.findById(catalogId);
    ...
    private static void updateProduct(Scanner scanner) {
        ...
        if (product != null) {
            ...
            String newName = ProductService.validateProductName(scanner);
            System.out.print("Nhập giá sản phẩm mới: ");
            double newPrice = ProductService.validateProductPrice(scanner);
            System.out.print("Nhập mô tả sản phẩm mới: ");
            String newDescription = scanner.nextLine();
            System.out.print("Nhập số lượng tồn kho mới ");
            int newStock = ProductService.validateProductStock(scanner);
